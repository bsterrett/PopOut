...
public class MinimaxThread extends Thread {
	...
	public MinimaxThread(final short empty_space_number,
			final short player_number, final short computer_number,
			final short[][] test_board_short, final int depth,
			final short turn, final String move) {
	...
	private final short minimax(final short[][] test_board_short,
		...
		short test_board_temp[][] = new short[p_column_count][p_row_count]; // paranoid
																			// sanitation
																			// of
																			// references,
																			// can
		...
		if (depth <= 0 || current_board.compute_win() != p_empty_space_number) {
			/*
			 * switch(p_heuristic){ case 1: return
			 * Search.evaluate_board_one(current_board); case 2: return
			 * Search.evaluate_board_two(current_board); case 3: return
			 * evaluate_board_three(current_board, move); case 4: return
			 * evaluate_board_four(current_board, move); case 101: return
			 * evaluate_move_one(move); case 102: return
			 * evaluate_move_two(current_board, move); default: return
			 * evaluate_board_four(current_board, move); }
			 */
			System.err
					.println("Need to make that static evaluation function or something!");
		}
